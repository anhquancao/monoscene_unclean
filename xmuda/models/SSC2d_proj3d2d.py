...
from xmuda.models.decoder3D_nyu import Decoder3DNYU
...
class SSC2dProj3d2d(pl.LightningModule):
    def __init__(self,
        ...
        assert context_prior in [None, "CRCP"]
        ...
        if self.corenet_proj is None:
            ...
            for scale in self.scales:
                key = str(scale)
                full_size = self.scene_sizes[0]
                self.projects[key] = Project2D3D(
                    (self.full_scene_size[0] // scale, self.full_scene_size[1] // scale, self.full_scene_size[2] // scale),
                    project_scale=self.project_scale,
                    dataset=self.dataset)
...
#                                                 context_prior=context_prior)
            self.net_3d_decoder = Decoder3DNYU(self.n_classes, 
                                               nn.BatchNorm3d, 
                                               n_relations=n_relations, 
                                               features=features, 
        ...
        if rgb_encoder == "UResNet":
            ...
            self.net_rgb = UEfficientNetEncoder.build(out_feature=feat_2D,
                                                      use_decoder=self.corenet_proj is None)
        # log hyperparameters
...
#        self.complet_evaluator = iouEval(12, [])

#        self.train_relation_dist = np.zeros(4)
#        self.val_relation_dist = np.zeros(4)
#        self.test_relation_dist = np.zeros(4)
    ...
    def forward(self, batch):
        ...
        if self.corenet_proj is None:
            x3ds = []
#            print(self.project_scale)
#            print(list(batch.keys()))
            for i in range(bs):
                x3d = None
                for scale in self.project_res:
                    scale = int(scale)
                    pix = batch["pix_{}".format(self.project_scale)][i].cuda()
                    valid_pix = batch["valid_pix_{}".format(self.project_scale)][i].cuda()
                    if x3d is None:
                        x3d = self.projects[str(scale)](x_rgb["1_" + str(scale)][i], pix // scale, valid_pix)
                    else:
                        x3d += self.projects[str(scale)](x_rgb["1_" + str(scale)][i], pix // scale, valid_pix)
                x3ds.append(x3d)
            ...
            for i in range(bs):
                for scale in [4, 8, 16]:
                    scale = int(scale)
                    pix = batch["pix_{}".format(scale)][i].cuda()
                    valid_pix = batch["valid_pix_{}".format(scale)][i].cuda()
                    input_dict[scale].append(self.projects[str(scale)](x_rgb["1_" + str(scale // 4)][i], pix // (scale // 4), valid_pix))

            for k in input_dict.keys(): 
                input_dict[k] = torch.stack(input_dict[k])
            input_dict["x_global"] = x_rgb['global']
