...
class ASPP2D(nn.Module):
  ...
  def __init__(self, planes, dilations_conv_list):
    ...
    self.conv1 = nn.ModuleList(
      [nn.Conv2d(planes, planes, kernel_size=3, padding=dil, dilation=dil, bias=False) for dil in dilations_conv_list])
...
#    x_in = x_in[:, None, :, :, :]
    ...
    for i in range(1, len(self.conv_list)):
      y += self.bn2[i](self.conv2[i](self.relu(self.bn1[i](self.conv1[i](x_in)))))
    x_features = y + x_in
    x_in = self.relu(x_features)  # modified

    logit = self.conv_classes(x_in)

    return logit, x_features


class LMSCNet(nn.Module):

  def __init__(self, class_num, output_scene_size, full_scene_size, in_channels=32):
    '''
    SSCNet architecture
...
#    input = torch.squeeze(input, dim=1).permute(0, 2, 1, 3)  # Reshaping to the right way for 2D convs [bs, H, W, D]
    ...
    if self.full_scene_size[0] != self.output_scene_size[0]:
        out_scale_1_4__3D = self.seg_head_1_4(out_scale_1_4__2D.unsqueeze(1))
        return out_scale_1_4__3D
    # out_scale_1_8__3D = self.seg_head_1_8(out_scale_1_8__2D)
    
#    out_scale_1_4__2D = out_scale_1_4__2D.unsqueeze(1)
    ...
    out_scale_1_1__2D = F.relu(self.conv1_1(out)).unsqueeze(1)
#    out_scale_1_4__2D = out_scale_1_4__2D.unsqueeze(1)
  ...
  def compute_loss(self, pred, target, class_weights):
    '''
