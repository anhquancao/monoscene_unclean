...
class MLP(nn.Module):
    def __init__(self, in_channels, feature, dim_coords, out_channels, n_layers):
        super().__init__()
        self.layers = []
        for layer in range(n_layers):
            self.layers.append(
                nn.Sequential(
                    nn.Linear(feature + dim_coords, feature),
...
class CPImplicitPairwise(nn.Module):
    def __init__(self, in_channels, out_channels, feature, size, n_classes=12, non_empty_ratio=0.2, max_k=256, bn_momentum=0.0003):
        ...
        self.pred_relate_logit = MLP(feature, 256, 3, 1, n_layers=5)

#        f = 32 # res_1_16
        self.pred_P_logit = MLP(feature, 256, 6, self.n_relations, n_layers=6)
...
#        print(self.coords.min(1), self.coords.max(1))
        self.coords[0, :] = self.coords[0, :] / 14 - 0.5
        self.coords[1, :] = self.coords[1, :] / 8 - 0.5
        self.coords[2, :] = self.coords[2, :] / 14 - 0.5

  
    def forward(self, input, masks, pts_cam, non_empty_ratio, max_k):
        ret = {}
...
#            if self.training: 
            perm = torch.randperm(N).type_as(xi).long()
            topk_indices = perm[:k] 
#            else:
...
#            xi_with_indices = xi_repeat + masked_pe_tiles # N * k, c 
            abs_rel_coords = torch.cat([abs_coord_repeat,relative_coord], dim=1) # N * k, c 
...
#            P = P.T.reshape(self.n_relations, N, k).reshape(-1, k) # 78 * N, k
