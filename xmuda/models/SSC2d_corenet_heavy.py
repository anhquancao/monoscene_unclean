...
class SSC2dCorenetHeavy(pl.LightningModule):
    def __init__(self,
                 preprocess_dir,
                 n_classes,
                 class_names,
        ...
        self.unroll = nn.Sequential(
            nn.ConvTranspose3d(features[0], features[0], kernel_size=(5, 3, 5), stride=1, padding=0),
            nn.BatchNorm3d(features[0]),
            nn.ReLU(),
            nn.ConvTranspose3d(features[0], features[0], kernel_size=3, stride=3, padding=0),
            nn.BatchNorm3d(features[0]),
            nn.ReLU(),
            nn.ConvTranspose3d(features[0], features[0], kernel_size=4, stride=4, padding=0),
            nn.BatchNorm3d(features[0]),
            nn.ReLU(),
        )
    ...
    def forward(self, batch):
        ...
        x2d = x_rgb["1_1"] # 200, 480, 640
        x3d_global = x2d.reshape(bs, x2d.shape[1], -1).mean(2)
        x3d_global_unrolled = self.unroll(x3d_global[:, :, None, None, None])

        input_dict = {
            "x3d": x3d_global_unrolled,
        }
