...
class MLP(nn.Module):
    def __init__(self, in_channels, feature, dim_coords, dim_project, out_channels, n_layers):
        ...
        self.project_input = nn.Sequential(
            ...
            nn.ReLU(), # Old model need to remove this
...
#            Bottleneck3D(feature, feature // 4, bn_momentum=bn_momentum, norm_layer=nn.BatchNorm3d, dilation=[4, 4, 4]),
        ...
        self.pred_relate_logit = MLP(feature, feature, 3, 32, 1, n_layers=4)
#        self.pred_relate_logit = MLP(feature, 256, 3, 32, 1, n_layers=4)

#        f = 32 # res_1_16
        self.pred_P_logit = MLP(feature, feature, 6, 64, self.n_relations, n_layers=6)
...
#        self.resize = nn.Conv3d(feature * (self.n_relations + 1), out_channels, kernel_size=1)
        self.resize = nn.Conv3d(feature * self.n_relations, out_channels, kernel_size=1)
...
#        self.transformer_encoder = nn.TransformerEncoder(transformer_encoder_layer, num_layers=4)
    ...
    def forward(self, input, masks, pts_cam, max_k, k_indices=None):
        ret = {}
...
#                _, topk_indices = torch.topk(relate_prob.squeeze(), k, dim=0)
#            else:
#                topk_indices = k_indices[i]
#                k = len(topk_indices)

            # weighted random sampling
            k = int(np.minimum(N, max_k))
            topk_indices = torch.multinomial(relate_prob.squeeze(), k, replacement=True) # (k,)
#            topk_indices = torch.multinomial(relate_prob.squeeze()/torch.sum(relate_prob), k, replacement=True) # (k,)
...
#            b_size = N * k // self.n_batchs
#            P_logit = torch.zeros(N * k, self.n_relations).type_as(xi_repeat)
#            for l in range(self.n_batchs - 1):
#                start = l * b_size
#                if l != self.n_batchs - 2:
#                    end = (l + 1) * b_size
#                else:
#                    end = N * k
#                P_logit[start:end, :] = self.pred_P_logit(xi_repeat[start:end, :], 
#                                                          abs_rel_coords[start:end, :]) # N * k, 78 
            P_logit = self.pred_P_logit(xi_repeat, abs_rel_coords) # N * k, 78 
            P_logits.append(P_logit)

            P = F.softmax(P_logit, dim=1) # N * k, n_relation 
            ...
            x_context_item = x_context_item.reshape(self.n_relations, N, -1) # 4, N, c

            # remove empty empty
            x_context_item = x_context_item[[0, 1, 3], :, :] # 3, N, c

            x_context[i, :, mask] = x_context_item.permute(0, 2, 1).reshape(-1, N)

        
        x_context = x_context.reshape(bs, self.n_relations * c, self.size[0], self.size[1], self.size[2]) 
        x = torch.cat([input, x_context], dim=1)
        x = self.resize(x)
#        x_context = x_context.reshape(bs, (self.n_relations - 1) * c, self.size[0], self.size[1], self.size[2]) 
#        x = torch.cat([input, x_context], dim=1)
#        x = self.resize(x)
